...
class SudoAuthService {
  ...
  // Initialise l'auth en chargeant les valeurs persistées (web refresh)
  static Future<void> init() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      _token = prefs.getString('sudo_token');
      _email = prefs.getString('sudo_email');
      _role = prefs.getString('sudo_role');
    } catch (e) {
      debugPrint('[SudoAuthService] init error: ${e.toString()}');
    }
  }
  ...
  Future<void> loginInWithEmailAndPassword(
    ...
    try {
      final prefs = await SharedPreferences.getInstance();
      if (_token != null) await prefs.setString('sudo_token', _token!);
      if (_email != null) await prefs.setString('sudo_email', _email!);
      if (_role != null) await prefs.setString('sudo_role', _role!);
    } catch (e) {
      debugPrint('[SudoAuthService] save prefs error: ${e.toString()}');
    }
  }
  ...
  Future<void> signOut() async {
    ...
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.remove('sudo_token');
      await prefs.remove('sudo_email');
      await prefs.remove('sudo_role');
    } catch (e) {
      debugPrint('[SudoAuthService] clear prefs error: ${e.toString()}');
    }
  }
}
